        ...
        <div class="card">
          ...
          >
            <div style="display: flex">
              ...
              <div v-if="box.isDeleted" class="col">
...
export default {
  ...
  data() {
    return {
      isEditable: false,
      booleanTable: ["Yes", "No"],
      statusTable: ["In Progress", "Pending", "Completed"],
  ...
  computed: {
    arrowLeft() {
      let arrowUp = "turn-arrow-up";
      let arrowDown = 'turn-arrow-down'
      if (this.turnArrow === true) {
        return arrowUp;
      } else {
        return arrowDown;
      }
    ...
    timeChange() {
      ...
      if (time <= today && status != "Completed") {
        return {
          "bg-danger": true,
          visible: true,
        };
      } else if (time <= inThreeDays && status != "Completed") {
        return {
          "bg-warning": true,
        };
  ...
  mounted() {
    this.$store.dispatch("refreshStyle");
  },
  methods: {
    removeBox(idNumber) {
...
.turn-arrow-up {
-webkit-animation: rotate-up 0.5s ease-out forwards;
}

.turn-arrow-down {
-webkit-animation: rotate-down 0.5s ease-out forwards;
}
